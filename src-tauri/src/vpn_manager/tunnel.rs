...
///
/// Steps:
/// 1. Build a `WireGuardConfig` from the supplied pre-generated key pair and
///    parameters, then write it to [`CONFIG_PATH`] with `0o600` permissions.
/// 2. Resolve the sidecar binary directory and validate all three binaries.
/// 3. Execute `wg-quick up` via `osascript` with administrator privileges.
/// 4. Delete the config file in both success and failure paths.
///
/// The caller is responsible for generating and managing the key pair lifecycle.
...
pub async fn tunnel_up(
    ...
    // 1. Build config and write to disk (key pair is pre-generated by caller).
