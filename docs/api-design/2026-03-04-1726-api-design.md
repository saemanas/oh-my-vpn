...
status: complete
created: "2026-03-04"
updated_at: "2026-03-06T01:30:00+07:00"
upstream:
...
| IPC-AL | Menu Bar UI → App Lifecycle | Tauri IPC (JSON) | Request/Response | Cross-cutting §9 |
...
### E. IPC-AL: Menu Bar UI → App Lifecycle

| Endpoint | Method | Description | Source FR |
| --- | --- | --- | --- |
| `quit_app` | IPC Command | Exit the application gracefully after disconnect | Cross-cutting §9 |
| `cancel_quit` | IPC Command | Cancel a pending quit confirmation, resume hide-on-blur | Cross-cutting §9 |

**Backend events** (emitted to frontend via `AppHandle::emit`):

| Event | Payload | Description |
| --- | --- | --- |
| `quit-requested` | `()` | Emitted when user quits while a VPN session is active; frontend shows confirmation dialog |
...
type SessionStatus = {
  provider: Provider;
  region: string;
  regionDisplayName: string; // human-readable region name (e.g., "Falkenstein, DE")
...
3. Generate ephemeral WireGuard server + client key pairs
4. Build cloud-init script (embed WG server private key + client public key)
5. Create server with cloud-init (WireGuard + firewall)
6. Wait for server to be ready
7. Delete SSH key from provider and memory
8. Write WireGuard config using pre-generated client key pair (permission 600)
9. Execute `wg-quick up` via sudo (ADR-0001)
10. Delete WireGuard config file
11. Create ActiveSession file
12. Update UserPreferences (lastProvider, lastRegion)

> **Note**: WireGuard key pairs must be generated before server creation (step 3) because the cloud-init script needs the WG server private key and client public key embedded at provisioning time. Server Lifecycle owns both key pairs; VPN Manager receives the pre-generated client key pair for tunnel setup.
...
1. Execute `wg-quick down` via sudo (best-effort -- continues if tunnel already down)
2. Destroy server via provider API
3. Verify server deletion via provider API (cross-cutting §8.B)
4. Delete ActiveSession file
5. On persistent failure after 3 retries: return error with manual deletion guide (provider console URL)
...
- `"reconnect"`: Re-establish WireGuard tunnel to existing server (reuse original WireGuard keys stored in session -- server's wg0.conf hardcodes the original client public key as peer)
...
```rust
#[tauri::command]
async fn get_preferences() -> Result<UserPreferencesResponse, AppError>
```

`UserPreferencesResponse` is the frontend-facing projection of the internal `UserPreferences` struct, excluding the backend-only `schemaVersion` field.
...
Exit the application gracefully. Called after the frontend completes the disconnect flow in a quit-while-connected scenario.
...
- Clears the `QUIT_PENDING` flag (re-enables hide-on-blur)
- Calls `AppHandle::exit(0)` to terminate the application

#### `cancel_quit`

Cancel a pending quit confirmation. Called when the user dismisses the quit-while-connected dialog.
...
- Clears the `QUIT_PENDING` flag so the window resumes hide-on-blur behavior
- No other side effects

---

### G. TRAIT-CP: CloudProvider Trait
...
sequenceDiagram
    ...
    rect rgb(230, 245, 255)
        note over SL,Cloud: SSH Key + WG Keys + Server Provisioning
        SL->>SL: Generate SSH key pair (Ed25519)
        SL->>KA: retrieve_credential(provider)
        KA-->>SL: API key
        SL->>PM: create_ssh_key(apiKey, publicKey)
        PM->>Cloud: Register SSH key
        Cloud-->>PM: sshKeyId
        PM-->>SL: sshKeyId

        SL->>SL: Generate WG server + client key pairs
        SL->>SL: Build cloud-init (WG server privkey + client pubkey)
        SL->>PM: create_server(apiKey, region, sshKeyId, cloudInit)
        PM->>Cloud: Create instance
        Cloud-->>PM: ServerInfo (IP, ID)
        PM-->>SL: ServerInfo

        SL->>PM: delete_ssh_key(apiKey, sshKeyId)
    ...
    rect rgb(230, 255, 230)
        ...
        SL->>VM: tunnel_up(clientKeyPair, serverIp, serverPublicKey, address, dns)
        VM->>VM: Write config from pre-generated key pair (permission 600)
...
sequenceDiagram
    ...
    VM-->>SL: Tunnel torn down (best-effort)
...
> Messages in the User-Facing Message column are **defaults**. Frontend may rewrite for UX clarity as long as the intent is preserved.
