...
erDiagram
    ...
    USER_PREFERENCES {
        ...
        Provider lastProvider "Last used provider (enum)"
...
| `regionDisplayName` | `String` | No | Human-readable region name (e.g., `Falkenstein, DE`). Defaults to `region` if absent |
...
| `lastProvider` | `Provider?` | No | Last used provider enum (`hetzner`, `aws`, `gcp`) for expert shortcut pre-selection |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `privateKey` | `[u8; 32]` | Yes | Ed25519 private key seed (zeroed on drop) |
| `publicKey` | `[u8; 32]` | Yes | Ed25519 public key (formatted as OpenSSH for registration) |

Generated before server provisioning (ADR-0004). The public key is formatted as OpenSSH (`ssh-ed25519 AAAA...`) and registered with the cloud provider. The provider-side key ID (`providerKeyId`) is tracked externally by `ConnectCleanup` in the connect flow -- not stored on the key pair struct itself -- to enable auto-cleanup on provisioning failure. Both the local key pair and the provider-side key are deleted after cloud-init completes (or on provisioning failure). The private key is never written to disk. `Zeroize` + `ZeroizeOnDrop` ensures memory is cleared.
